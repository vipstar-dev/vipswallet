...
import (
	...
	"github.com/vipstar-dev/vipswallet/internal/legacy/rename"
)
...
func getAddressKey(addr vipsutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]vipsutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]vipsutil.Address),
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]vipsutil.Address)
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (vipsutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ChangeAddress(bs *BlockStamp) (vipsutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) LastChainedAddress() vipsutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) Address(a vipsutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) SetSyncStatus(a vipsutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *vipsutil.WIF, bs *BlockStamp) (vipsutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := vipsutil.Hash160(wif.SerializePubKey())
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (vipsutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(vipsutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
type WalletAddress interface {
	// Address returns a vipsutil.Address for the backing address.
	Address() vipsutil.Address
...
func (s *Store) ActiveAddresses() map[vipsutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[vipsutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the vipsutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]vipsutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// vipswallet shall refuse to open any unencrypted addresses.  This
...
// that vipswallet fits into an unused space left by Armory's key store file
// format.
...
type PubKeyAddress interface {
	...
	ExportPrivKey() (*vipsutil.WIF, error)
}
...
func newBtcAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *btcAddress, err error) {
	...
	address, err := vipsutil.NewAddressPubKeyHash(vipsutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
func (a *btcAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	addr, err := vipsutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *btcAddress) Address() vipsutil.Address {
	return a.address
}
...
func (a *btcAddress) ExportPrivKey() (*vipsutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := vipsutil.NewWIF((*btcec.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := vipsutil.Hash160(script)

	address, err := vipsutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := vipsutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a vipsutil.AddressScriptHash for a btcAddress.
func (sa *scriptAddress) Address() vipsutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []vipsutil.Address {
	return sa.addresses
}
