...
func (s *WithdrawalStatus) Fees() vipsutil.Amount {
	return s.fees
}
...
func (o OutBailmentOutpoint) Amount() vipsutil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() vipsutil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() vipsutil.Amount {
		return vipsutil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total vipsutil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total vipsutil.Amount) {
...
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, txmgrNs walletdb.ReadBucket, chainHeight int32, dustThreshold vipsutil.Amount) (
...
func (w *withdrawal) finalizeCurrentTx() error {
	...
	for _, txOut := range tx.outputs {
		...
		amtFulfilled := vipsutil.Amount(0)
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := vipsutil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := vipsutil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold vipsutil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold vipsutil.Amount) (*WithdrawalStatus, error) {
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *vipsutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addrmgrNs, addresses[0].(*vipsutil.AddressScriptHash))
	if err != nil {
