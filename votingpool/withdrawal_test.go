...
import (
	...
	vp "github.com/vipstar-dev/vipswallet/votingpool"
)
...
func TestStartWithdrawal(t *testing.T) {
	...
	checkWithdrawalOutputs(t, status, map[string]vipsutil.Amount{address1: 4e6, address2: 1e6})

	if status.Fees() != vipsutil.Amount(1e3) {
		t.Fatalf("Wrong amount for fees; got %v, want %v", status.Fees(), vipsutil.Amount(1e3))
	}
...
func checkWithdrawalOutputs(
	t *testing.T, wStatus *vp.WithdrawalStatus, amounts map[string]vipsutil.Amount) {
