...
func (p *Pool) DepositScriptAddress(seriesID uint32, branch Branch, index Index) (vipsutil.Address, error) {
	script, err := p.DepositScript(seriesID, branch, index)
	if err != nil {
		return nil, err
	}
...
func (p *Pool) addressFor(script []byte) (vipsutil.Address, error) {
	scriptHash := vipsutil.Hash160(script)
	return vipsutil.NewAddressScriptHashFromHash(scriptHash, p.manager.ChainParams())
}
...
func (p *Pool) DepositScript(seriesID uint32, branch Branch, index Index) ([]byte, error) {
	...
	pks := make([]*vipsutil.AddressPubKey, len(pubKeys))
	for i, key := range pubKeys {
		...
		pks[i], err = vipsutil.NewAddressPubKey(pubkey.SerializeCompressed(),
...
func (p *Pool) addUsedAddr(ns, addrmgrNs walletdb.ReadWriteBucket, seriesID uint32, branch Branch, index Index) error {
	...
	encryptedHash, err := p.manager.Encrypt(waddrmgr.CKTPublic, vipsutil.Hash160(script))
	if err != nil {
...
func (p *Pool) getUsedAddr(ns, addrmgrNs walletdb.ReadBucket, seriesID uint32, branch Branch, index Index) (
	...
	addr, err := vipsutil.NewAddressScriptHashFromHash(hash, mgr.ChainParams())
	if err != nil {
