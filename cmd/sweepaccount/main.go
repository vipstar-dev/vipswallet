...
var (
	walletDataDirectory = vipsutil.AppDataDir("vipswallet", false)
	newlineBytes        = []byte{'\n'}
)
...
func makeInputSource(outputs []btcjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue vipsutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]vipsutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := vipsutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(vipsutil.Amount) (vipsutil.Amount, []*wire.TxIn, []vipsutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	var totalSwept vipsutil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := vipsutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount vipsutil.Amount) bool {
	return amount >= 0 && amount <= vipsutil.MaxSatoshi
}
