...
func (w *Wallet) MakeMultiSigScript(addrs []vipsutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*vipsutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*vipsutil.AddressScriptHash, error) {
	var p2shAddr *vipsutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = vipsutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*vipsutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
