...
func NewRecoveryState(recoveryWindow uint32) *RecoveryState {
	...
	return &RecoveryState{
		recoveryWindow:   recoveryWindow,
		scopes:           scopes,
		watchedOutPoints: make(map[wire.OutPoint]vipsutil.Address),
	}
}
...
func (rs *RecoveryState) WatchedOutPoints() map[wire.OutPoint]vipsutil.Address {
...
type BranchRecoveryState struct {
	...
	addresses map[uint32]vipsutil.Address
...
func NewBranchRecoveryState(recoveryWindow uint32) *BranchRecoveryState {
	return &BranchRecoveryState{
		recoveryWindow:  recoveryWindow,
		addresses:       make(map[uint32]vipsutil.Address),
...
func (brs *BranchRecoveryState) AddAddr(index uint32, addr vipsutil.Address) {
	brs.addresses[index] = addr
}

// GetAddr returns the address derived from a given child index.
func (brs *BranchRecoveryState) GetAddr(index uint32) vipsutil.Address {
...
func (brs *BranchRecoveryState) Addrs() map[uint32]vipsutil.Address {
