...
import (
	...
	h "github.com/vipstar-dev/vipswallet/internal/helpers"
	"github.com/vipstar-dev/vipswallet/wallet/internal/txsizes"
)
...
// InputSourceError.
type InputSource func(target vipsutil.Amount) (total vipsutil.Amount, inputs []*wire.TxIn,
	inputValues []vipsutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []vipsutil.Amount
	TotalInput      vipsutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb vipsutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []vipsutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := vipsutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
