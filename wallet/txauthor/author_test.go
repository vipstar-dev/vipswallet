...
func p2pkhOutputs(amounts ...vipsutil.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	currentInputValues := make([]vipsutil.Amount, 0, len(unspents))
	f := func(target vipsutil.Amount) (vipsutil.Amount, []*wire.TxIn, []vipsutil.Amount, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			u := unspents[0]
			unspents = unspents[1:]
			nextInput := wire.NewTxIn(&wire.OutPoint{}, nil, nil)
			currentTotal += vipsutil.Amount(u.Value)
			currentInputs = append(currentInputs, nextInput)
			currentInputValues = append(currentInputValues, vipsutil.Amount(u.Value))
		}
...
func TestNewUnsignedTransaction(t *testing.T) {
	tests := []struct {
		...
		Outputs          []*wire.TxOut
		RelayFee         vipsutil.Amount
		ChangeAmount     vipsutil.Amount
