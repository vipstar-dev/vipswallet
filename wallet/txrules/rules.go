...
const DefaultRelayFeePerKb vipsutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb vipsutil.Amount) vipsutil.Amount {
	...
	relayFee := vipsutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount vipsutil.Amount, scriptSize int, relayFeePerKb vipsutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb vipsutil.Amount) bool {
	...
	return IsDustAmount(vipsutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb vipsutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > vipsutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb vipsutil.Amount, txSerializeSize int) vipsutil.Amount {
	fee := relayFeePerKb * vipsutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > vipsutil.MaxSatoshi {
		fee = vipsutil.MaxSatoshi
	}
