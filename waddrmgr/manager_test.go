...
import (
	...
	"github.com/vipstar-dev/vipswallet/snacl"
...
func testExternalAddresses(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i := 0; i < len(expectedExternalAddrs); i++ {
			...
			utilAddr, err := vipsutil.NewAddressPubKeyHash(
				pkHash, chainParams,
			)
			if err != nil {
...
func testImportPrivateKey(tc *testContext) bool {
	...
	if tc.create {
		for i, test := range tests {
			...
			wif, err := vipsutil.DecodeWIF(test.in)
			if err != nil {
...
func testImportScript(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			// Use the Address API to retrieve each of the expected
			// new addresses and ensure they're accurate.
			utilAddr, err := vipsutil.NewAddressScriptHash(test.in,
				chainParams)
			if err != nil {
...
func testMarkUsed(tc *testContext) bool {
	...
	for i, test := range tests {
		...
		switch test.typ {
		case addrPubKeyHash:
			addr, err = vipsutil.NewAddressPubKeyHash(addrHash, chainParams)
		case addrScriptHash:
			addr, err = vipsutil.NewAddressScriptHashFromHash(addrHash, chainParams)
		default:
			panic("unreachable")
		}
		if err != nil {
...
func testLookupAccount(tc *testContext) bool {
	...
	for i, addr := range expectedAddrs {
		addr, err := vipsutil.NewAddressPubKeyHash(addr.addressHash,
...
func TestScopedKeyManagerManagement(t *testing.T) {
	...
	_, ok := externalAddr[0].Address().(*vipsutil.AddressScriptHash)
	if !ok {
	...
	_, ok = internalAddr[0].Address().(*vipsutil.AddressWitnessPubKeyHash)
	if !ok {
		t.Fatalf("wrong type: %T", externalAddr[0].Address())
	}
