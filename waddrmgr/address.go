...
// Address returns the vipsutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() vipsutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*vipsutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return vipsutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	if compressed {
		...
		pubKeyHash = vipsutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address vipsutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := vipsutil.NewAddressWitnessPubKeyHash(
		...
		address, err = vipsutil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = vipsutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *vipsutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() vipsutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := vipsutil.NewAddressScriptHashFromHash(
