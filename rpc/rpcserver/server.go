...
//
//   https://github.com/vipstar-dev/vipswallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/vipstar-dev/vipswallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"github.com/vipstar-dev/vipswallet/internal/zero"
...
// as well as establishing a RPC connection to a vipsd consensus server.
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := vipsutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	var totalAmount vipsutil.Amount
	for _, output := range unspentOutputs {
		...
		totalAmount += vipsutil.Amount(output.Output.Value)

		if req.TargetAmount != 0 && totalAmount > vipsutil.Amount(req.TargetAmount) {
			break
		}
	}

	var changeScript []byte
	if req.IncludeChangeScript && totalAmount > vipsutil.Amount(req.TargetAmount) {
...
//   sendrawtransaction, so connection errors to vipsd could result in the tx
