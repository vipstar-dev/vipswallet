...
(account 0) from a testnet3 wallet listening on `localhost:32915` in several
different languages:
...
import (
	...
	pb "github.com/vipstar-dev/vipswallet/rpc/walletrpc"
...
var certificateFile = filepath.Join(vipsutil.AppDataDir("vipswallet", false), "rpc.cert")
...
func main() {
	...
	conn, err := grpc.Dial("localhost:32915", grpc.WithTransportCredentials(creds))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	fmt.Println("Spendable balance: ", vipsutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/vipswallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/vipswallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by vipswallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.vipswallet/rpc.cert"s;
    }();
    ...
    auto channel = grpc::CreateChannel("localhost:32916", creds);
...
```PowerShell
PS> & protoc.exe -I \Path\To\vipswallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by vipswallet).
            ...
            var channel = new Channel("localhost:32916", new SslCredentials(cert));
            try
            {
...
var certPath = path.join(process.env.HOME, '.vipswallet', 'rpc.cert');
...
var client = new walletrpc.WalletService('localhost:32915', creds);
...
```bash
$ protoc -I /path/to/vipstar-dev/vipswallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.vipswallet', 'rpc.cert')
    ...
    channel = implementations.secure_channel('localhost', 32916, creds)
