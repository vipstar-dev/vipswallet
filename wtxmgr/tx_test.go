...
import (
	...
	_ "github.com/vipstar-dev/vipswallet/walletdb/bdb"
)
...
var (
	...
	TstRecvTx, _                    = vipsutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = vipsutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *vipsutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := vipsutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstDoubleSpendTx := vipsutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := vipsutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: vipsutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: vipsutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: vipsutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: vipsutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: vipsutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: vipsutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := vipsutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance vipsutil.Amount,
	...
	checkBalance(vipsutil.Amount(initialBalance), false)
	checkBalance(vipsutil.Amount(initialBalance), true)
	...
	checkBalance(vipsutil.Amount(changeAmount), true)
