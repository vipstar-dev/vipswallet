...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []vipsutil.Address) error {
	c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]vipsutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []vipsutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]vipsutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []vipsutil.Address, outPoints map[wire.OutPoint]vipsutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]vipsutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
